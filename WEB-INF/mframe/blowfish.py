...
The author auf xxxBase64 is:
   Wilhelm Nagy   <wilhelm.nagy@gmail.com
...
class Blowfish:
    ...
    Public functions:
        ...
        def decryptBase64(self,data,salt):
            Decrypt a String. If salt is not None a 'salted' String 
            is added on begin auf String. Return the string in base64.

        def encryptBase64(self, data, salt):
            encrypted a Message in Base64. Optional salted (length in salt)
            is removed.
    ...
    def getSalt(self,length):
       """
       Gibt aus einem Alphabet eine eine zufaelligen String
       von Zeichen zurueck.
       ...
       random.shuffle(lst)
       return ''.join(lst[0:length])

    def encryptBase64(self,text,salt=None):
       """
       Verschluessen eines Textes beliebiger Laenge.
       Optional kann der Text auch noch 'versalzen' werden.
       Die Laenge des Salzeintrages wird im Parameter salt bestimmt.

       @param  salt     Saltzstring Laenge
       @return Verschluesselter String als Base 64 kodiert.

       """
       if salt is not None:
         text = self.getSalt(salt)+text
         

       self.initCTR()
       crypted = self.encryptCTR(text)
       return base64.b64encode(crypted)

    def decryptBase64(self,crypted,salt=None):
       """
       Entschluesseln

       @param  crypted        Verschluesselte Nachricht
                              als Base64
       @param  salt           Salzstring Laenge
...
if __name__ == '__main__':
   ...
   for i in range(100,132):
      print i,' -> ',
      crypted = cipher.encryptBase64(str(i),salzlaenge)
      print crypted,' -> ',
      print cipher.decryptBase64(crypted,salzlaenge)
      p=''
   print cipher.decryptBase64(p,salzlaenge)
